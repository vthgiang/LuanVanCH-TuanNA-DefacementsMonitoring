module.exports = function(TOKEN, data){
    ...
    function walk(elem){
        ...
        if(elem.nodeType === 1){    // element
            ...
            if(!isInvisible(elem)){
                ...
                if(node.name === 'img'){
                ...
                }else if(~['style', 'script'].indexOf(node.name)){
                    if(!(IGNORE_TEXT_SELECTORS && elem.webkitMatchesSelector(IGNORE_TEXT_SELECTORS))){
                        node.rect = [1,1,1,1];
                        node.child.push({
                            name: '#',
                            text: md5(elem.outerHTML.replace(/\s+/g, ' '))
                        });
                    }
                    ...
                    for(var i = 0, len = elem.childNodes.length; i < len; i++){
                        ...
                        if(filter(child, elem)){    // recursion
                            ...
                            if(typeof vdom !== 'undefined' && (~['script', 'style'].indexOf(vdom.name) || vdom.style !== false)){
    ...
    var ret = walk(document.querySelector(ROOT));
    return ret;
